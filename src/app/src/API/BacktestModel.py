...
# START_DATE   = "2025-02-01"       # inclusive Feb 1, 2025
# END_DATE     = datetime.today().strftime("%Y-%m-%d")  # up to today

# MODEL_DIR    = "./bert_finetuned" # output_dir from Trainer
# BASE_MODEL   = "ProsusAI/finbert"  # base model for tokenizer

# # ─── 1) FETCH backtest raw data ────────────────────────────────────────────
# def fetch_backtest():
#     for sym in TICKERS:
#         path = os.path.join(BACKTEST_DIR, f"{sym}.csv")
#         print(f"Fetching {sym} {START_DATE}→{END_DATE} → {path}")
...
# # ─── 2) PROCESS & build inference dataset ─────────────────────────────────
...
#         df = df.dropna(subset=["target","SMA20","EMA20","RSI14","MACD","OBV","ATR14"])
...
# # ─── 3) FIND & LOAD BEST CHECKPOINT ────────────────────────────────────────
# def get_best_checkpoint(model_dir):
#     # 1) try trainer_state.json
...
#     # 2) fallback: highest-numbered checkpoint-*
#     ckpts = [
...
# # ─── 4) RUN INFERENCE & COLLECT PREDICTIONS ────────────────────────────────
# def infer(df):
#     preds, probs, trues = [], [], []
...
#         probs.append(float(softmax[idx]))
...
#     print("1) Fetching backtest raw data…")
#     fetch_backtest()

#     print("\n2) Building backtest dataset…")
#     backtest_df = build_backtest_dataset()
#     print(f"  Total backtest examples: {len(backtest_df)}")
